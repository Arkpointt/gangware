...
def main():
	...
	calibration_complete = config_manager.get('calibration_complete', fallback='False') == 'True'
	...
	# Initialize and start Worker thread (pass overlay for status updates)
	worker = Worker(config_manager, state_manager, vision_controller, input_controller, task_queue, status_callback=overlay)
	worker.start()

	# Optional UI demo task
	ui_demo = config_manager.get('ui_demo', fallback='False') == 'True'
	if ui_demo:
		# Enqueue a demo task that updates status periodically
